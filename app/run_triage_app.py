#!/usr/bin/env python3
"""
TriageFlow: App-Launched Incident Triage

This script is designed to be launched from the TriageFlow Streamlit app.
It reads configuration from the Domino dataset instead of the project directory.

Usage:
    python run_triage_app.py --config-path /mnt/data/project/configs/config_20241208_143022.yaml --provider openai --vertical financial_services -n 5
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the TriageFlow incident triage pipeline with configuration from dataset."
    )
    parser.add_argument(
        "--config-path",
        type=str,
        required=True,
        help="Path to the configuration YAML file in the dataset"
    )
    parser.add_argument(
        "--provider",
        type=str,
...
def main():
    ...
    print("=" * 80)
    print("TriageFlow: App-Launched Incident Triage")
    print("=" * 80)
    ...
    # Extract config filename for tagging
    ...
    # Load sample incidents from project directory
    ...
    # Run the pipeline with config from dataset
    results = []
    run_id = None

    with DominoRun(agent_config_path=args.config_path, custom_summary_metrics=aggregated_metrics) as run:
        mlflow.set_tag("mlflow.runName", run_name)
        mlflow.set_tag("config_file", config_filename)
        mlflow.set_tag("launched_from", "app")
    ...
    print(f"Configuration used: {args.config_path}")
    print("=" * 80)
